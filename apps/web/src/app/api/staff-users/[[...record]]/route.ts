import {staffOperationDiagnostic,type StaffOperationPhase} from '../../../../lib/staff-operation-diagnostic';
...
async function handle(request:Request){let phase:StaffOperationPhase='AUTHENTICATION';try{
 ...
 const id=match[1];
 if(request.method==='GET'&&!id){phase='LIST';return Response.json({items:await listAccounts(runtime.authPool,state.principal)},{headers});}
 ...
 phase='INPUT';const input=await readJson(request);phase='WRITE';const result=await writeAccount(runtime.authPool,state.principal,id,input);return Response.json(result,{status:id?200:201,headers});
}catch(error){const known=error instanceof LedgerError?error:new LedgerError('STAFF_OPERATION_FAILED',500);if(known.status>=500){const diagnostic=staffOperationDiagnostic(error,phase);console.error(JSON.stringify(diagnostic));}return Response.json({error:known.code},{status:known.status,headers});}}
