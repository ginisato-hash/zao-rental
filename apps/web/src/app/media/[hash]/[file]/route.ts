...
export async function GET(req:Request,p:{params:Promise<{hash:string;file:string}>}){const {hash,file}=await p.params,empty=()=>new Response(null,{status:404,headers:{'Cache-Control':'no-store'}});if(new URL(req.url).search||!/^[a-f0-9]{64}$/.test(hash)||!/^\d+\.(webp|jpg)$/.test(file))return empty();const path='/media/'+hash+'/'+file,r=publicRuntime();if(!r)return empty();
 if(productionRequested()&&!getProductionRuntime()?.configuration.flags.media)return empty();
 try{const result=await releasedDerivative(path,{async readPrivateObject(digest){if(productionRequested())return getProductionRuntime()?.readDerivative(digest)??null;return (await r.readPool.query('SELECT bytes FROM content_media_objects WHERE sha256=$1',[digest])).rows[0]?.bytes??null;}},async()=>{const m=(await publicModels()).find(m=>m.media.variants.some(v=>v.src===path)),v=m?.media.variants.find(v=>v.src===path);return m&&v?{path,sha256:hash,type:v.type as 'image/webp'|'image/jpeg',revision:JSON.stringify(m)}:null;});if(!result)return empty();return new Response(new Uint8Array(result.bytes),{headers:{...result.headers,'Content-Type':result.type,'Content-Length':String(result.bytes.length)}});}catch{return empty();}}
