...
export default async function Page({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const runtime=getRuntime(),content=avatarRuntime();if(!runtime||!content)notFound();
 let payloads;
 try{payloads=await loadAvatarPreview(runtime.recommendationPool,s.principal,q.preview,q.member,content,new Date());}catch(e){if(e instanceof HoldError&&e.status===403)notFound();throw e;}
