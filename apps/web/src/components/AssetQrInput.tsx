...
export function AssetQrInput({onAsset,disabled=false}:{onAsset:(id:string)=>Promise<void>;disabled?:boolean}){
 ...
 const online=useSyncExternalStore(cb=>{window.addEventListener('online',cb);window.addEventListener('offline',cb);return()=>{window.removeEventListener('online',cb);window.removeEventListener('offline',cb);};},()=>navigator.onLine,()=>true);
 const ready=useSyncExternalStore(subscribe,()=>true,()=>false);
 const [pending,setPending]=useState(false),[starting,setStarting]=useState(false),[running,setRunning]=useState(false),[manual,setManual]=useState(''),[message,setMessage]=useState('');useEffect(()=>{callback.current=onAsset;},[onAsset]);
 ...
 useEffect(()=>{const offline=()=>{stop();setMessage('通信が切れました。保存済みと未確認の結果を照合してから再開してください。');};window.addEventListener('offline',offline);return()=>window.removeEventListener('offline',offline);},[stop]);
 async function emit(raw:string){if(!navigator.onLine){setMessage('通信未確認のため送信していません。');return;}const id=assetIdFromQr(raw);if(!id){setMessage('用品の個体IDを読み取ってください。');return;}if(busy.current)return;busy.current=true;setPending(true);setMessage('照合結果を待っています。保存済みかどうかはまだ確認できません。');try{await callback.current(id);if(alive.current)setMessage('コードを読み取りました。保存・受領の結果は照合画面で確認してください。');}catch{if(alive.current){stop();setMessage('照合できませんでした。未確認の要求を確認してから再開してください。');}}finally{busy.current=false;if(alive.current)setPending(false);}}
 async function start(){if(disabled||starting||running||!ready||!navigator.onLine)return;const own=++generation.current;setStarting(true);setMessage('');last.current='';
  ...
  }catch(e){if(alive.current&&generation.current===own){stop();setMessage((e as Error).name==='NotAllowedError'?'カメラの許可がありません。設定を確認するか個体IDを手入力してください。':(e as Error).name==='NotFoundError'?'カメラが見つかりません。個体IDを手入力できます。':'カメラを開始できませんでした。個体IDを手入力できます。');}}
 }
 ...
 <div style={{display:'flex',flexWrap:'wrap',gap:8,minHeight:44}}><button type="button" style={{minHeight:44,minWidth:44}} disabled={!ready||disabled||!online||starting||running} onClick={()=>void start()}>カメラを開く</button><button type="button" style={{minHeight:44,minWidth:44}} disabled={!starting&&!running} onClick={stop}>カメラを止める</button></div>
 <label>用品の個体ID<input value={manual} onChange={e=>setManual(e.target.value)} maxLength={80} disabled={!ready||disabled||!online||pending} style={{display:'block',minHeight:44,maxWidth:'100%',boxSizing:'border-box'}}/></label><button type="button" style={{minHeight:44,minWidth:44}} disabled={!ready||disabled||!online||pending||!manual} onClick={()=>void emit(manual)}>入力したIDを照合へ送る</button><p role="status">{starting?'カメラの許可を待っています。':message}</p></section>;
}
