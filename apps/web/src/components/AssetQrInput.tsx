...
export function AssetQrInput({onAsset,disabled=false}:{onAsset:(id:string)=>Promise<void>;disabled?:boolean}){
 ...
 async function emit(raw:string){if(!navigator.onLine){setMessage('通信未確認のため送信していません。');return;}const id=assetIdFromQr(raw);if(!id){setMessage('用品の個体IDを読み取ってください。');return;}if(busy.current)return;busy.current=true;try{await callback.current(id);if(alive.current)setMessage('コードを読み取りました。保存・受領の結果は照合画面で確認してください。');}catch{if(alive.current){stop();setMessage('照合できませんでした。未確認の要求を確認してから再開してください。');}}finally{busy.current=false;}}
 ...
 <label>用品の個体ID<input value={manual} onChange={e=>setManual(e.target.value)} maxLength={80} disabled={!ready||disabled||!online} style={{display:'block',minHeight:44,maxWidth:'100%',boxSizing:'border-box'}}/></label><button type="button" style={{minHeight:44,minWidth:44}} disabled={!ready||disabled||!online||!manual} onClick={()=>void emit(manual)}>入力したIDを照合へ送る</button><p role="status">{starting?'カメラの許可を待っています。':message}</p></section>;
}
