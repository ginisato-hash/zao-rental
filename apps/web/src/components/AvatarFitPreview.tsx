...
export function AvatarFitPreview({visualization,direction,locale,mediaScope}:{visualization?:AvatarVisualizationV1|null;direction:Direction;locale:Locale;mediaScope?:GuestAvatarScope}){
 ...
 const scope=mediaScope?guestAvatarPath(mediaScope):null;if(mediaScope&&!scope)return null;
 const boxes=layout.boxes.map(box=>scope?{...box,src:'/guest-avatar-media/'+scope+'/'+box.visual.id+'/'+box.visual.derivativeSha256}:box);
 return <section className="avatar-fit" lang={locale} aria-label={ja?'サイズの見た目を確認':'Visual size reference'}>
  ...
  <div className="avatar-stage-wrap"><div className="avatar-stage" data-avatar-stage style={{aspectRatio:layout.width+'/'+layout.height}}>
   ...
   {boxes.map(box=><Layer key={box.src} box={box}/>)}</div></div>
