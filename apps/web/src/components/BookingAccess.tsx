...
export function ConfirmedBooking(){
 const [view,setView]=useState<View|null>(null),[message,setMessage]=useState('読み込み中…'),[busy,setBusy]=useState(false),[reading,setReading]=useState(true),[pendingBookingId,setPendingBookingId]=useState<string|null>(null);const ticket=useRef(0),revoking=useRef(false);
 async function reload(){if(revoking.current)return;const n=++ticket.current;setReading(true);setView(null);try{const v=await request('');if(n===ticket.current){setView(v);setMessage('');}}catch{if(n===ticket.current)setMessage('予約の閲覧権がないか、失効・期限切れです。予約を変更する権限はありません。');}finally{if(n===ticket.current){setPendingBookingId(pendingRevoke());setReading(false);}}}
 useEffect(()=>{
 ...
 return <section aria-label="保存済み予約"><h1>予約確認・QR</h1><p>開発用の合成予約です。実決済・本番予約ではありません。</p>{view&&<><p>{view.state} / {view.mode}</p><p>{view.period.startDate} → {view.period.endDate}</p><p>{view.pickupStore} → {view.returnStore}</p><p>元の返却期限: <time>{view.dueAt}</time></p><p>保存済み参考総額: {view.totalJpy} JPY（請求確定不可）</p><picture><img src={view.qrImage} width={240} height={240} alt="保存済み予約QR"/></picture><p>QRは予約の識別子です。貸出には店舗スタッフによる認証・権限確認が必要です。</p></>}{message&&<p role="status">{message}</p>}<button disabled={busy||reading} onClick={()=>void reload()}>予約を再読込</button><button disabled={busy||reading||(!view&&!pendingBookingId)} onClick={async()=>{
