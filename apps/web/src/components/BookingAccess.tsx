...
export function ConfirmedBooking({locale}:{locale:string}){
 ...
 async function reload(){if(revoking.current)return;const n=++ticket.current;setReading(true);setView(null);try{const v=await request('');if(n===ticket.current){setView(v);setMessage('');}}catch{if(n===ticket.current)setMessage(t('予約の閲覧権がないか、失効・期限切れです。予約を変更する権限はありません。','You do not have viewing access to this booking, or it has expired or been revoked. You cannot change the booking.'));}finally{if(n===ticket.current){setPendingBookingId(pendingRevoke());setReading(false);}}}
 useEffect(()=>{
  let cancelled=false;const n=++ticket.current;
  request('').then(v=>{if(!cancelled&&n===ticket.current){setView(v);setMessage('');}})
   .catch(()=>{if(!cancelled&&n===ticket.current)setMessage(t('予約の閲覧権がないか、失効・期限切れです。','You do not have viewing access to this booking, or it has expired or been revoked.'));})
