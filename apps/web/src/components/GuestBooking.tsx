...
export function GuestBooking({locale}:{locale:Locale}){const ja=locale==='ja',t=(j:string,e:string)=>ja?j:e;
 ...
 function guardNav(e:React.MouseEvent){if(dirty&&!window.confirm(t('保存されていない入力があります。移動すると内容が失われます。続けますか？','You have unsaved input. Leaving now will lose it. Continue?')))e.preventDefault();}
 ...
 {dirty&&<p role="status">{t('保存されていない変更があります。移動すると失われます。','You have unsaved changes. Leaving now will lose them.')}</p>}
 ...
 {step===0&&<><div className="guest-grid"><label>{t('利用開始日','Start date')}<input type="date" value={input.period.startDate} onChange={e=>setInput(i=>({...i,period:{...i.period,startDate:e.target.value,endDate:i.period.endDate&&i.period.endDate>=e.target.value?i.period.endDate:e.target.value}}))}/></label><label>{t('利用終了日','End date')}<input type="date" min={input.period.startDate||undefined} value={input.period.endDate} onChange={e=>setInput(i=>({...i,period:{...i.period,endDate:e.target.value}}))}/></label><label>{t('利用枠','Rental slot')}<select aria-label={t('利用枠','Rental slot')} value={input.period.slot} onChange={e=>setInput(i=>({...i,period:{...i.period,slot:e.target.value}}))}>{SLOT_OPTIONS.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>{(['pickupStore','returnStore'] as const).map(f=><label key={f}>{f==='pickupStore'?t('受取店舗','Pickup store'):t('返却店舗','Return store')}<select aria-label={f==='pickupStore'?t('受取店舗','Pickup store'):t('返却店舗','Return store')} value={input[f]} onChange={e=>setInput(i=>({...i,[f]:e.target.value}))}>{Object.entries(STORE_LABEL).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>)}</div><button disabled={!input.period.startDate||!input.period.endDate||input.period.endDate<input.period.startDate} onClick={()=>pushStep(1)}>{t('用品を選ぶ','Choose equipment')}</button></>}
 ...
 </>}</main><GuestRecovery locale={locale}/></div>;
}
