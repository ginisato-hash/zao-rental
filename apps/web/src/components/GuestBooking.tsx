...
export function GuestBooking({locale}:{locale:Locale}){const ja=locale==='ja',t=(j:string,e:string)=>ja?j:e;
 ...
 const dirty=touched&&step<2&&JSON.stringify(input)!==(savedInputJson??JSON.stringify(blankInput()));
 ...
 {draft.hold&&<p>{draft.booking?.state==='CONFIRMED_DEV'?t('確認済み予約は元の契約期間の在庫保護を維持。参考：初期HOLD期限','Confirmed booking retains contract-period protection. Initial HOLD expiry for reference'):t('在庫HOLD期限','Inventory HOLD expiry')}: <time dateTime={draft.hold.expiresAt}>{jstTime(draft.hold.expiresAt)}</time>{draft.booking?.state!=='CONFIRMED_DEV'&&<> / {holdStateText(ja,draft.hold.state)}</>}</p>}{draft.quote&&<p>{t('保存済み見積','Saved estimate')}: {money(draft.quote.snapshot.totalJpy)} / {quoteValidityText(ja,draft.quote.validity)}</p>}</>;
