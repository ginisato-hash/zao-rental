...
type Hold={transferAttention?:string|null;id:string;conditions:HoldConditions;state:string;expiresAt:string;version:number;paymentState:string;period:{startsAt:string;dueAt:string;occupancyStartsAt:string;occupancyEndsAt:string};history:{event:string;actor:string}[]};
...
function Workspace({stamp,stores,canEdit}:{stamp:string;stores:StoreId[];canEdit:boolean}){
 ...
 <div className="hold-actions"><button disabled={blocked} onClick={()=>void perform(selected?`/${selected.id}/availability`:'/availability',conditions(),false)}>在庫を照会</button><button disabled={blocked||!canEdit} onClick={()=>void perform(selected?`/${selected.id}/amend`:'',{requestKey:crypto.randomUUID(),conditions:conditions(),...(selected?{expectedVersion:selected.version}:{})},true)}>{selected?'条件を一括変更':'グループを仮押さえ'}</button>{selected&&<button disabled={blocked||!canEdit} onClick={()=>void perform(`/${selected.id}/cancel`,{requestKey:crypto.randomUUID()},true)}>仮押さえを取り消す</button>}<button disabled={blocked} onClick={()=>{setSelected(null);reservation.current=null;setMessage('新しい要求');}}>新しい要求</button></div>
