...
const PICKUP_ELIGIBLE=['CONFIRMED_DEV','COMPLETED_DEV'];
...
export function StaffHome(p:{stamp:string;stores:StoreId[];canBookingView:boolean;canCheckout:boolean;canReturn:boolean;canOperationsView:boolean;canManageStaff:boolean}){return <StaffSessionBoundary stamp={p.stamp}><Home {...p}/></StaffSessionBoundary>;}
function Home({stamp,stores,canBookingView,canCheckout,canReturn,canOperationsView,canManageStaff}:{stamp:string;stores:StoreId[];canBookingView:boolean;canCheckout:boolean;canReturn:boolean;canOperationsView:boolean;canManageStaff:boolean}){
 ...
 useEffect(()=>{if(canReturn&&returnStore)void returnsReq.load<Returns>('/api/custody/returns?store='+returnStore,setReturns);
 // eslint-disable-next-line react-hooks/exhaustive-deps
 },[canReturn,returnStore]);
 ...
 {canBookingView&&<BookingSearchInput onBooking={openBooking}/>}
 {canBookingView&&<section aria-label="本日の予約"><h2>本日</h2><p className="staff-secondary">本日が利用期間に含まれる予約（{today} JST）。完全な入出庫予定表ではありません。</p>
  ...
  {bookings&&(todays.length?<div className="staff-card-grid">{todays.map(b=><article className="staff-card" key={b.id}><p><strong>{b.period!.startDate} → {b.period!.endDate}</strong> · {b.period!.slot}</p><p>{b.display_name??'（氏名未取得）'}</p><p>{STATE_LABEL[b.state]??b.state}</p><p>{b.pickup_store}→{b.return_store} · {money(b.total_jpy)}</p>{PICKUP_ELIGIBLE.includes(b.state)&&<button className="staff-card-primary" onClick={()=>openBooking(b.id)}>この予約を開く</button>}</article>)}</div>:<p>本日が利用期間に含まれる予約はありません。</p>)}
 </section>}
 {canCheckout&&<section aria-label="貸出・受付"><h2>貸出・受付</h2><p>予約QR・検索、または本日のカードから開くと、この画面に予約IDが引き継がれます。</p><Link href="/staff/rentals">貸出・受付の画面を開く</Link></section>}
 {canReturn&&<section aria-label="返却の進行状況"><h2>返却の進行状況</h2><label>店舗<select aria-label="返却店舗" value={returnStore} onChange={e=>setReturnStore(e.target.value as StoreId)}>{stores.map(s=><option key={s} value={s}>{s}</option>)}</select></label>
  ...
  {returns&&<p>進行中の返却バッチ {returns.batches.length}件 · 受領済み・検品待ち {inspectionPending}件</p>}
 ...
 <details className="staff-secondary-links"><summary>その他の管理機能</summary><nav><a href="/staff/ledger">道具の台帳</a><a href="/admin/inventory">棚卸・CSV投入</a><a href="/staff/amendments">変更・返金依頼</a><a href="/staff/transfers">店舗間移動</a><a href="/staff/quotes">見積</a><a href="/staff/holds">期間在庫・HOLD</a><a href="/staff/recommendations">サイズ推薦</a><a href="/staff/wear">ウェアの数量貸出・返却</a><a href="/staff/password">パスワード変更</a>{canManageStaff&&<a href="/staff/users">スタッフ管理</a>}</nav></details>
 </main>;
}
