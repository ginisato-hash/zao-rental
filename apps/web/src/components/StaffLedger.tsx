...
export function StaffLedger({stamp,canEdit,canEditGlobal,canManageStaff}:{stamp:string;canEdit:boolean;canEditGlobal:boolean;canManageStaff:boolean}){
 ...
 return <StaffSessionBoundary stamp={stamp}><div className="staff-toolbar"><span>開発用DB · {canEdit?'台帳の閲覧・編集':'台帳の閲覧のみ'}</span><nav><a href="/staff/transfers">店舗間移動</a><a href="/staff/recommendations">サイズ推薦</a> · <a href="/staff/quotes">見積へ</a> · <a href="/staff/holds">期間在庫・HOLD</a><a href="/staff/password">パスワード変更</a>{canManageStaff&&<a href="/staff/users">スタッフ管理</a>} <button onClick={()=>void logout()}>ログアウト</button></nav></div><LedgerWorkspace client={client} canEdit={canEdit} canEditGlobal={canEditGlobal}/></StaffSessionBoundary>;
}
