...
import {AssetLabelSheet} from '../AssetLabelSheet';
 ...
 return <main className="ledger-shell">
  ...
  <div className="ledger-workspace">
   <section className="ledger-list" aria-label={tabs[resource]}>
    ...
    <div className="ledger-table-wrap"><table><thead><tr><th>道具・商品</th><th>区分 / サイズ</th><th>{resource==='poles'?'台帳数量':'管理単位'}</th><th>店舗 / 状態</th><th>詳細</th></tr></thead><tbody>{page.items.map(row=><tr key={row.id}><td><strong>{row.name}</strong><span>{label(row.family)} · {label(row.sourceKind)}</span></td><td>{label(row.age)} / {label(row.tier)}<span>{row.size??'—'}</span></td><td>{resource==='assets'?`1 ${row.unit==='BOARD'?'枚':'組'}`:resource==='poles'?`${row.quantity} ペア`:resource==='bundles'?'構成定義のみ':'マスター'}<span>{row.family==='SKI'&&resource==='assets'?'左右ラベル2枚・1 Asset':resource==='poles'&&row.atStorePairs!==undefined?`実在店 ${row.atStorePairs} / 貸出中 ${row.outPairs} ペア`:row.bslStatus==='UNVERIFIED'?'BSL要確認':resource==='bundles'?'物理在庫を増やしません':''}</span></td><td>{label(row.custody??row.storeId)}<span className="ledger-status">{row.custody==='RETURNED_PENDING_INSPECTION'?'返却後の検品は未完了':label(row.status)}</span></td><td><button aria-label={`詳細：${row.name} ${row.size??row.code}`} onClick={()=>void open(row)}>見る ↗</button></td></tr>)}</tbody></table></div>
   ...
   <aside className="ledger-detail" aria-label="台帳詳細">{detail?<><p className="ledger-kicker">REGISTER DETAIL</p><h2>{detail.name}</h2><span className="ledger-pill">{label(detail.sourceKind)}</span><dl><dt>不変のID</dt><dd className="ledger-id">{detail.id}</dd><dt>区分・サイズ</dt><dd>{label(detail.age)} / {label(detail.tier)} / {detail.size??'—'}</dd>{detail.storeId&&<><dt>所在地・受領状態</dt><dd>{label(detail.custody??detail.storeId)}<small>店舗移動は基本編集の対象外</small></dd></>}{detail.resource==='assets'&&<><dt>管理単位</dt><dd>{detail.unit==='BOARD'?'1枚 = 1 Asset':'左右1組 = 1 Asset'}<small>同じIDのラベル：{detail.labelCopies}枚</small></dd><dt>スキー用BSL</dt><dd><span>{detail.bslStatus==='UNVERIFIED'?'要確認・未記録':detail.bslStatus==='RECORDED'?`${detail.bslMm} mm`:'対象外'}</span><small>{detail.bslEvidence||'靴サイズからの推測・DIN計算は行いません'}</small></dd></>}{detail.resource==='poles'&&<><dt>台帳数量</dt><dd>{detail.quantity} ペア（1ペア＝2本）{detail.atStorePairs!==undefined&&<small>実在店 {detail.atStorePairs} / 貸出中 {detail.outPairs} ペア。期間の予約可能数ではありません。</small>}</dd></>}<dt>出典</dt><dd>{detail.sourceDocument}<small>{detail.sourceLocator}</small></dd><dt>備考</dt><dd>{detail.notes||'—'}</dd></dl>{detail.components&&<div className="ledger-components">{detail.components.map(c=><p key={c.family}>{label(c.family)} × {c.quantity} {c.unit==='BOARD'?'枚':'ペア'}</p>)}<small>同じ年齢区分・クラスの構成条件です。予約確保は行いません。</small></div>}{editable&&<button className="ledger-primary" onClick={()=>setForm('update')}>基本情報を更新</button>}{detail.resource==='assets'&&<AssetLabelSheet id={detail.id} family={detail.family} size={detail.size}/>}<h3>登録・更新履歴</h3><ol className="ledger-history">{detail.history.map((h,i)=><li key={i}><strong>{h.action==='REGISTER'?'登録':'更新'} · {h.reason}</strong><span>{new Date(h.occurredAt).toLocaleString('ja-JP')}</span></li>)}</ol>{detail.locations.map((h,i)=><p key={i} className="ledger-footnote">{h.event==='INITIAL_REGISTRATION'?'初期所属':h.event==='RETURN_RECEIPT'?'返却受領':'移動受領'}：{label(h.storeId)} / 履歴保持</p>)}</>:<><p className="ledger-kicker">ONE RECORD, ONE TOOL</p><h2>道具の履歴を、<br/>ここに。</h2><p>一覧の「見る」から、個体ID、出典、BSLの確認状況と変更履歴を開けます。</p><div className="ledger-rule">スキー左右のラベルは同じID。<br/>2枚貼っても、台帳は1ペアです。</div><p className="ledger-footnote">ウェアは上下・サイズ別の数量管理です。衣類の個体IDやQRは発行しません。実在庫の総数・内訳は未確認です。</p></>}</aside>
