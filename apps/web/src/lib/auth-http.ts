...
export function authHandler(auth:StaffAuth|null,pool:Pool|null,origin:string|null,loginPool:Pool|null){return async(request:Request)=>{
 ...
 try{
  ...
  if(login){
   ...
   try{
    ...
    if(response.ok&&member){
     ...
     if(member)await client.query("UPDATE staff_members SET failed_login_count=failed_login_count+1,locked_until=CASE WHEN failed_login_count+1>=5 THEN now()+interval '15 minutes' ELSE NULL END WHERE id=$1",[member.id]);
  ...
  if(change){const state=await resolveStaff(auth,pool,request.headers);if(state.status!=='authorized')return reply('AUTHENTICATION_REQUIRED',401);}
  let response=await auth.handler(forward(change?{...body,revokeOtherSessions:false}:reset?body:{}));
  if(change&&response.ok){
   // DB trigger revoked every old session. Let the library clear the old cookie as well.
   response=await auth.handler(forward({},`${origin}/api/auth/sign-out`));
  }else if(logout&&response.ok)await pool.query("SELECT staff_log('LOGOUT',$1,$1)",[session?.user.id??null]);
