...
const allowed=(s:StaffState)=>s.status==='authorized'&&s.principal.permissions.includes('BOOKING_VIEW');
/** Phase4 is staff-only, including bytes. Client release/rights claims are never read. */
export function avatarMediaHandler(state:(headers:Headers)=>Promise<StaffState>,reader:()=>AvatarMediaReader|null,clock:()=>Date=()=>new Date()){
 return async(req:Request)=>{
  ...
  try{
   ...
   const r=reader();if(!r)return empty();const bytes=await avatarDerivative(r,parts[2]!,parts[3]!,clock);if(!bytes)return empty();
