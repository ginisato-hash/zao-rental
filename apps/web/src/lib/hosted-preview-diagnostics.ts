export const hostedPreviewStartupStages=['PREVIEW_IDENTITY','CONFIG_PARSE','DB_CONNECT_GUEST','DB_CONNECT_HOLD','DB_CONNECT_PRICING','DB_CONNECT_RECOMMENDATION','DB_CONNECT_CONTENT_READ','DB_CONNECT_AVATAR_READ','DB_IDENTITY','TLS','GUEST_SECURITY_INIT','AVATAR_SECURITY_INIT','R2_INIT','READY'] as const;
...
export function hostedPreviewUnavailable(error:unknown){
 const stage=error instanceof HostedPreviewStartupError&&hostedPreviewStartupStages.includes(error.stage)?error.stage:'UNCLASSIFIED';
