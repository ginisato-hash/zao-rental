...
async function createHostedPreview(){
 let stage:HostedPreviewStartupStage='PREVIEW_IDENTITY';
 const pools={} as Record<Phase6Service,Pool>;
 try{
  const origin=previewOrigin(process.env);stage='CONFIG_PARSE';
  const c=parseHostedPreview(process.env.ZAO_HOSTED_PREVIEW_RUNTIME,process.env);
  ...
  stage='READY';return {origin,contexts,service,security:{service:security,peer},avatar};
 }catch{await Promise.all(Object.values(pools).map(p=>p.end().catch(()=>{})));throw new HostedPreviewStartupError(stage);}
}
