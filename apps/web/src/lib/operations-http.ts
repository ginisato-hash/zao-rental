...
export function operationsHandler(state:(headers:Headers)=>Promise<StaffState>,factory:((id:OpsIdentity)=>OperationsContext)|null,origin:string){return async(request:Request)=>{try{
 ...
 if(request.method==='GET'){
  if([...url.searchParams.keys()].some(k=>!['id','store'].includes(k)))throw new FlowError('INVALID_QUERY',422);
  switch(path){case '/notifications':result=await new NotificationOperations(ctx).list(url.searchParams.get('store')??'');break;case '/amendment-options':result=await amend.options();break;case '/amendment':result=await amend.view(url.searchParams.get('id')??'');break;case '/financial':result=await money.summary(url.searchParams.get('id')??'',url.searchParams.get('store')??'');break;case '/inventory':result=await inventory.workspace(url.searchParams.get('store')??'');break;case '/stocktake':result=await inventory.get(url.searchParams.get('id')??'');break;case '/import-catalog':result=await inventory.importCatalog();break;default:throw new FlowError('NOT_FOUND',404);}
 }else{
  ...
  switch(path){case '/notification-resend':result=await new NotificationOperations(ctx).resend(body.requestKey,body.input);break;case '/amendment-quote':result=await amend.quote(body.requestKey,body.input);break;case '/amendment-accept':result=await amend.accept(body.requestKey,body.input);break;case '/refund':result=await money.requestRefund(body.requestKey,body.input);break;case '/stocktake-create':{const v=flowObject(body.input,['store']);if(typeof v.store!=='string')throw new FlowError('INVALID_STORE',422);result=await inventory.create(body.requestKey,v.store);break;}case '/stocktake-observe':result=await inventory.observe(body.requestKey,body.input);break;case '/stocktake-reconcile':result=await inventory.reconcile(body.requestKey,body.input);break;case '/import-stage':result=await inventory.stageImport(body.requestKey,body.input);break;case '/import-commit':result=await inventory.commitImport(body.requestKey,body.input);break;default:throw new FlowError('NOT_FOUND',404);}
 }
