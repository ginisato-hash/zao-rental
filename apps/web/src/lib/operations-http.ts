...
export function operationsHandler(state:(headers:Headers)=>Promise<StaffState>,factory:((id:OpsIdentity)=>OperationsContext)|null,origin:string){return async(request:Request)=>{try{
 ...
 if(request.method==='GET'){
  const allowed=path==='/exceptions'?['store','type','severity','ageHours','status','beforeTime','beforeId']:['id','store'];
  if([...url.searchParams.keys()].some(k=>!allowed.includes(k)))throw new FlowError('INVALID_QUERY',422);
  switch(path){case '/exceptions':{const q=url.searchParams,age=q.get('ageHours')??'0';if(!/^\d{1,4}$/.test(age))throw new FlowError('OPS_FILTER_INVALID',422);result=await new OperationsConsole(ctx).list({store:q.get('store')??'',type:q.get('type'),severity:q.get('severity'),ageHours:Number(age),status:q.get('status')??'UNACKNOWLEDGED',beforeTime:q.get('beforeTime'),beforeId:q.get('beforeId')});break;}case '/notifications':result=await new NotificationOperations(ctx).list(url.searchParams.get('store')??'');break;case '/amendment-options':result=await amend.options();break;case '/amendment':result=await amend.view(url.searchParams.get('id')??'');break;case '/financial':result=await money.summary(url.searchParams.get('id')??'',url.searchParams.get('store')??'');break;case '/inventory':result=await inventory.workspace(url.searchParams.get('store')??'');break;case '/stocktake':result=await inventory.get(url.searchParams.get('id')??'');break;case '/import-catalog':result=await inventory.importCatalog();break;default:throw new FlowError('NOT_FOUND',404);}
 }else{
