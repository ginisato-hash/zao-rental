...
export function quoteHandler(state:(h:Headers)=>Promise<StaffState>,service:(p:StaffPrincipal)=>QuoteService,origin:string){return async(request:Request)=>{try{
 ...
 if(request.method==='GET')return Response.json(path===''?await svc.list():path==='/catalog'?await svc.catalog():path==='/options'?await svc.options():path.startsWith('/draft/')?await svc.draftView(path.slice(7)):await svc.get(path.slice(1)),{headers});
 ...
 if(path==='/manage'){const body=exact(await readJson(request),['requestKey','input']);id(body.requestKey);return Response.json(await svc.manage(body.input,body.requestKey),{headers});}if(path==='/private-initialize')return Response.json(await svc.manage(await readJson(request)),{headers,status:201});if(path!=='')throw new PricingError('NOT_FOUND',404);
