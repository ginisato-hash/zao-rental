...
export function quoteHandler(state:(h:Headers)=>Promise<StaffState>,service:(p:StaffPrincipal)=>QuoteService,origin:string){return async(request:Request)=>{try{
 ...
 const body=exact(await readJson(request),['requestKey','input']);id(body.requestKey);return Response.json(await svc.create(body.requestKey,body.input),{headers,status:201});
