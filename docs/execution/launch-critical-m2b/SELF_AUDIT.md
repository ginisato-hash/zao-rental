...
classification:          M2B_F3_PREFIX_CORRECTION_READY_FOR_RECHECK
reviewedHeads:           38e6e40 (REQUEST_CHANGES F1-F4), 6ddf378 (F1/F2/F4 CLOSED, F3 residual)
```
...
| F3 residual MEDIUM | Derived roles were still selected with `rolname LIKE current_database()||'\_%'`. The database name's own underscores are LIKE wildcards, so the same-length foreign name `zaoxrentalyproductionztest_custody_executor` was selected too: coexisting it raised a *false* collision, and replacing the real role it was **silently** given the expected placeholder. Both reproduced on a live PostgreSQL under `6ddf378`. | Both predicates now use a literal prefix, `starts_with(rolname, current_database()||'_')`. Four targeted cases run against the owned local PostgreSQL with transaction/rollback: the expected role is mapped; the lookalike is excluded; renaming the real role to the lookalike is not associated with it and changes the fingerprint; coexistence raises no false collision and leaves the fingerprint untouched. Reverting the predicates makes the case fail. |
