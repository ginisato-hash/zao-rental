...
2026-09-14 JST確認。今回の停止地点はSquare applicationの有無をOwnerへ確認するところ。
実外部受入は全件NOT_RUN。P5のfixture/実ローカルDB成功を外部成功へ読み替えない。

まず [Square Step 1](SQUARE_STEPS.md#step-1--今お願いする操作) のみ実行する。
その後にsecret store、HTTPS receiver、Vercel、Square、R2、backupの順で、該当する判断だけを求める。
下記の資料ができたことは、接続・契約・権限・deployの承認ではない。

| 順序 | 資料 | 現在の状態 |
|---|---|---|
| 1 | [Squareを1画面ずつ確認](SQUARE_STEPS.md) | application UNCONFIRMED |
| 2 | [Secret store / HTTPS receiverの比較](SECRETS_AND_RECEIVER.md) | store UNSELECTED、receiver NOT_CREATED |
| 3 | [Vercel direct実環境受入](VERCEL_ACCEPTANCE.md) | preview提案のみ。deploy未承認 |
...
P6で製品コード・migration・機械policyは変更しない。PR3/Runnerは保留。
