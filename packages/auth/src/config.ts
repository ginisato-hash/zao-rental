...
export type DevelopmentRuntime = {origin:string; namespace:string; authSecret:string; authDb:Connection; ledgerDb:Connection; holdDb:Connection;transferDb:Connection;pricingDb:Connection};
export function parseRuntime(raw:string|undefined):DevelopmentRuntime|null {
 ...
 try {
  ...
  for(const [db,suffix] of [[c.authDb,'auth'],[c.ledgerDb,'ledger'],[c.holdDb,'hold'],[c.transferDb,'transfer'],[c.pricingDb,'pricing']] as const){if(db.host!=='127.0.0.1'||db.database!==c.namespace||db.user!==`${c.namespace}_${suffix}`||!db.password||!Number.isInteger(db.port)||db.port<20000||db.port>29000)throw 0;}
  if(c.authDb.port!==c.ledgerDb.port||c.authDb.port!==c.holdDb.port||c.authDb.port!==c.transferDb.port||c.authDb.port!==c.pricingDb.port)throw 0;
