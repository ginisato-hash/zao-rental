...
export function previewOrigin(env:Readonly<Record<string,string|undefined>>){
 ...
 if(env.VERCEL!=='1'||env.VERCEL_ENV!=='preview'||env.VERCEL_TARGET_ENV!=='preview'||env.ZAO_AVATAR_PHASE6!=='PROTECTED_PREVIEW_V1'||env.VERCEL_PROJECT_ID!==phase6Project||env.VERCEL_GIT_COMMIT_REF!==phase6Branch||!host||!/^zao-rental-avatar-preview-[a-z0-9]{9}-zao-food-map\.vercel\.app$/.test(host))throw Error('PHASE6_PREVIEW_REQUIRED');
