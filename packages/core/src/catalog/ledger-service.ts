...
export class LedgerService {
  ...
  constructor(private readonly pool:Pool, private readonly principal:LedgerPrincipal|null) {this.scope=ledgerAccess(principal);}
  ...
  async list(resource:Resource,filters:LedgerFilters={}) {
    ...
    for(const key of ['storeId','age','tier','size','status'] as const)if(filters[key]!==undefined){values.push(filters[key]);clauses.push(`data->>'${key}'=$${values.length}`);}
