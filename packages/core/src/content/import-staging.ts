...
export function stageStockImport(text:string,sheet:string,variants:ImportVariant[],prior:Record<string,string>,catalogRevision:string){
 ...
 const raw=csvRows(text);if(raw.shift()?.join(',')!==header.join(','))throw new ContentInputError('IMPORT_HEADER');
 ...
 raw.forEach((original,n)=>{const row=n+2;if(original.length!==header.length){unresolved.push({row,codes:['IMPORT_COLUMNS']});return;}
  ...
  const value={documentSha256:sourceSha256,locator:sheet+'!row'+row,sourceKind,intent,modelId,season,variantId,manufacturerSku,quantity:quantity===''?null:Number(quantity),unit,assetIds:ids?ids.split('|').map(s=>s.trim()):[],storeId:storeId||null} as StockSource;
 ...
 const material={schemaVersion:1,sourceSha256,sheet,catalogRevision,staged,unresolved,plan};return {...material,stageSha256:digest(material)};
}
