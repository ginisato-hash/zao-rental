...
export interface PhotoJobStore{transaction<T>(fn:(jobs:Record<string,PhotoJob>)=>Promise<T>):Promise<T>;putPrivateObject(sha256:string,bytes:Buffer):Promise<void>}
...
export function photoCandidate(filename:string,knownOffers:string[]):PhotoItem['candidate']{
...
export class PrivatePhotoJobs{
 ...
 async prepare(subject:string,jobId:string,files:PhotoSelection[],offers:string[]){
