...
export class R2MediaProvider implements MediaProviderPort {
 readonly id='CLOUDFLARE_R2';private clients=new Set<S3Client>();private closed=false;readonly origin:string;
 constructor(private accountId:string,private bucket:string,private credentials:()=>Promise<R2Credential>,private purge:R2Purge,private now:()=>Date=()=>new Date(),private requestHandler?:S3ClientConfig['requestHandler']){
 ...
 /** One checked credential snapshot per operation. A long-lived SDK memoizer must
  * not keep signing new operations with a key that the resolver has rotated out.
  * In-flight operations use their already-approved snapshot; provider revocation
  * and existing signed-ticket expiry still require the external rotation procedure. */
 private async operation<T>(f:(client:S3Client,credential:R2Credential)=>Promise<T>){
  if(this.closed)throw new ContentInputError('R2_CREDENTIAL_UNAVAILABLE');const c=await this.check();if(this.closed)throw new ContentInputError('R2_CREDENTIAL_UNAVAILABLE');
  const client=new S3Client({region:'auto',endpoint:this.origin,forcePathStyle:true,maxAttempts:1,credentials:{accessKeyId:c.accessKeyId,secretAccessKey:c.secretAccessKey,expiration:c.expiresAt},...(this.requestHandler?{requestHandler:this.requestHandler}:{})});
  this.clients.add(client);try{return await f(client,c);}finally{this.clients.delete(client);client.destroy();}
 }
 ...
 async putIfAbsent(key:string,bytes:Uint8Array,metadata:{sha256:string;mime:string;visibility:'PRIVATE'}){
  if(!validKey(key)||!key.endsWith('/'+sha(bytes))||metadata.sha256!==sha(bytes)||metadata.visibility!=='PRIVATE'||bytes.byteLength<1||bytes.byteLength>32*1024*1024||!['image/jpeg','image/png','image/webp'].includes(metadata.mime))throw new ContentInputError('MEDIA_OBJECT_INVALID');
  try{await this.operation(client=>client.send(new PutObjectCommand({Bucket:this.bucket,Key:key,Body:bytes,IfNoneMatch:'*',ContentType:metadata.mime,CacheControl:'private, no-store',Metadata:{sha256:metadata.sha256}}),{abortSignal:AbortSignal.timeout(5000)}));}
 ...
 async readPrivate(key:string){if(!validKey(key))throw new ContentInputError('MEDIA_OBJECT_INVALID');return this.operation(async client=>{
  let response;try{response=await client.send(new GetObjectCommand({Bucket:this.bucket,Key:key}),{abortSignal:AbortSignal.timeout(5000)});}catch(e){if((e as {$metadata?:{httpStatusCode?:number}}).$metadata?.httpStatusCode===404)return null;throw new ContentInputError('MEDIA_FETCH_FAILED');}
 ...
 async signPrivateRead(key:string,expiresAt:Date){if(!validKey(key)||!key.startsWith('private/original/'))throw new ContentInputError('PRIVATE_MEDIA_DENIED');return this.operation(async(client,credential)=>{const now=this.now(),seconds=Math.floor((expiresAt.getTime()-now.getTime())/1000);
  ...
  const url=await getSignedUrl(client,new GetObjectCommand({Bucket:this.bucket,Key:key}),{expiresIn:seconds,signingDate:now});return {url,expiresAt};
 });}
 ...
 close(){this.closed=true;for(const client of this.clients)client.destroy();}
}
