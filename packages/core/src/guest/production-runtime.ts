import {issuePublicationAuthority,installPublicationAuthority,revokePublicationAuthority,type PublicationApproval,type PublicationAuthority} from '../../../auth/src/publication-authority';
import {CancellationRefundWorker} from '../payment/cancellation-refund-worker';
import type {CancellationRefundGateway} from '../payment/square-refund';
import {BookingNotificationWorker} from '../notification/worker';
import {productionNotificationDelivery,type ProductionNotificationBinding} from '../notification/production-delivery';
...
export type ProductionPaymentBinding={provider:'SQUARE';environment:'PRODUCTION';merchantId:string;applicationId?:string;credentials:()=>Promise<{environment:'PRODUCTION';merchantId:string;token:string;revoked:false}>;gateway:PaymentGateway};
export type ProductionRuntimeInput={configuration:unknown;approvedConfigurationSha256:string;
 ...
 refunds?:CancellationRefundGateway;
 notification?:ProductionNotificationBinding;
...
export async function composeProductionRuntime(input:ProductionRuntimeInput){
 let stage:StartupStage='FEATURE_FLAGS';const pools:Partial<Record<ProductionService,Pool>>={};let r2:R2MediaProvider|undefined;let validatedPayment:PaymentGateway|null=null;let publication:PublicationAuthority|undefined;
 try{
  ...
  if(input.refunds&&!refundWorker)throw new ProductionStartupError('PAYMENT');
  if(input.notification&&(!commercialIdentity||!recovery))throw new ProductionStartupError('BOOKING_ACCESS');
  const notifications=input.notification?new BookingNotificationWorker(required('operations'),c.deployment.origin,recovery!,await productionNotificationDelivery(input.notification,commercialIdentity)):null;
  ...
  return Object.freeze({configuration:c,staff,guest,service,access,recovery,avatar,refundWorker,notifications,readDerivative,payment:validatedPayment,contentReadPool:pools.content_read??null,
   ...
   safeStatus:()=>({APP:closed?'UNAVAILABLE':'READY',DB:closed?'UNAVAILABLE':Object.keys(pools).length>0?'READY':'OFF',GUEST:guest?'READY':'OFF',PAYMENT_ADAPTER:commercialIdentity?'CONFIGURED_COMMERCIAL':c.flags.payment?'CONFIGURED_ACTIVATION_PENDING':'OFF',WEBHOOK:!c.flags.payment?'OFF':c.payment?.webhookNotificationUrl?'CONFIGURED_ACTIVATION_PENDING':'UNCONNECTED',MEDIA:c.flags.media?'CONFIGURED':'OFF',NOTIFICATION:notifications?'CONFIGURED':'UNCONNECTED'} as const),
   async close(){if(closed)return;closed=true;if(publication)revokePublicationAuthority(publication);r2?.close();await Promise.all(Object.values(pools).map(p=>p.end().catch(()=>{})));}
  });
 }catch(error){if(publication)revokePublicationAuthority(publication);r2?.close();await Promise.all(Object.values(pools).map(p=>p.end().catch(()=>{})));throw error instanceof ProductionStartupError?error:new ProductionStartupError(stage);}
}
