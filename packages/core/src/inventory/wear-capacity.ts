...
export async function wearCapacityDetailed(c:Conn,conditions:HoldConditions,now:Date,ignore:string|null,excludeKeys?:ReadonlySet<string>){
 ...
 for(const r of requirements){
  ...
  const ok=increments.every(({day,n})=>n+(claims.find(x=>x.pool_id===pool.id&&x.day===day)?.quantity??0)<=pool.ready);
...
export async function wearCapacity(c:Conn,conditions:HoldConditions,now:Date,ignore:string|null){
 const d=await wearCapacityDetailed(c,conditions,now,ignore);
...
export async function writeWearClaims(c:Conn,holdId:string,conditions:HoldConditions,now:Date,excludeKeys?:ReadonlySet<string>){
 const plan=await wearCapacityDetailed(c,conditions,now,holdId,excludeKeys);if(!plan.feasible)throw new HoldError('WEAR_CAPACITY_CHANGED',409);
