...
export class FinancialOperations{
 ...
 private async row(c:OpsConnection,kind:Kind,id:string){flowId(id);const r=(await c.query<FinancialRow>(`SELECT * FROM ${table(kind)} WHERE id=$1`,[id])).rows[0];if(!r)throw new FlowError('FINANCIAL_REQUEST_NOT_FOUND',404);await this.booking(c,r.booking_id,r.acting_store);await this.ctx.authorize(kind==='refund'?'REFUND_OVERRIDE':'RENTAL_AMEND',r.acting_store?[r.acting_store]:[]);return r;}
