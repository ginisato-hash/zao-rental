...
export interface SquareTransport{readonly environment:'SANDBOX'|'PRODUCTION';readonly merchantId:string;send(call:SquareCall):Promise<{status:number;body:unknown}>;}
export class SquarePaymentGateway implements PaymentGateway{
 readonly kind:'SQUARE_UNCONNECTED'|'SQUARE_PRODUCTION';
 private readonly origin:string;
 constructor(private transport:SquareTransport,private source:(attemptId:string,signal:AbortSignal)=>Promise<string>,private timeoutMs=5000){if(!['SANDBOX','PRODUCTION'].includes(transport.environment)||!transport.merchantId||!Number.isSafeInteger(timeoutMs)||timeoutMs<1||timeoutMs>30000)throw new FlowError('SQUARE_CONFIGURATION_INVALID',503);this.origin=transport.environment==='PRODUCTION'?SQUARE_PRODUCTION_ORIGIN:SQUARE_SANDBOX_ORIGIN;this.kind=transport.environment==='PRODUCTION'?'SQUARE_PRODUCTION':'SQUARE_UNCONNECTED';}
 ...
 private async call(request:PaymentRequest,method:'GET'|'POST',path:string,body?:ReturnType<typeof squareCreateBody>):Promise<PaymentObservation>{
  ...
  try{const r=await this.deadline(signal=>this.transport.send({method,url:this.origin+path,version:SQUARE_VERSION,...(body?{body}:{}),signal}),method==='POST'?'PAYMENT_RESULT_UNKNOWN':'PAYMENT_LOOKUP_UNAVAILABLE');
