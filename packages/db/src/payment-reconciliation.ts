...
export class PgPaymentReconciliation implements PaymentReconciliationRepository,PaymentContextReader{
 constructor(private readonly pool:InboxPool,private readonly target?:{merchantId:string;paymentId:string}){}
 ...
 dispatch(environment:WebhookEnvironment,limit:number){return this.tx(async c=>(await c.query<{n:number}>(this.target?'SELECT payment_reconciliation.dispatch_target($1,$2,$3,$4) AS n':'SELECT payment_reconciliation.dispatch($1,$2) AS n',this.target?[environment,limit,this.target.merchantId,this.target.paymentId]:[environment,limit])).rows[0]!.n);}
 claimBatch(environment:WebhookEnvironment,workerId:string,limit:number){return this.tx(async c=>{
  const rows=(await c.query<{claim:ReconciliationClaim}>(this.target?'SELECT payment_reconciliation.claim_target($1,$2,$3,$4,$5) AS claim':'SELECT payment_reconciliation.claim($1,$2,$3) AS claim',this.target?[environment,workerId,limit,this.target.merchantId,this.target.paymentId]:[environment,workerId,limit])).rows;
