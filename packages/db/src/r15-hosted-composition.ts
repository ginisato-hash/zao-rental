...
export function r15HostedComposition(pools:{dispatcher:InboxPool;worker:InboxPool;projector:InboxPool;diagnostic:InboxPool},env:Readonly<Record<string,string|undefined>>,inputTarget:R15WorkerTarget,provider:PaymentTruthProvider){
 ...
 const once:PaymentTruthProvider={async lookupPayment(input){if(providerInvoked)throw new Error('R15_LOOKUP_ALREADY_INVOKED_DO_NOT_RETRY');providerInvoked=true;return provider.lookupPayment(input);}};
