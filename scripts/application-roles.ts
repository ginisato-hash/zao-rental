...
export async function provisionApplicationRoles(owner:Pool,identity:{namespace:string;database:string;dbPort:number}) {
 ...
 for(const suffix of ['auth','ledger','hold','transfer']) {
 ...
 const authDb=connections[0]!,ledgerDb=connections[1]!,holdDb=connections[2]!,transferDb=connections[3]!;
 ...
 return {authDb,ledgerDb,holdDb,transferDb,authPool,ledgerPool,holdPool,transferPool,async close(){await Promise.all([closeAuth(),closeLedger(),closeHold(),closeTransfer()]);}};
}
