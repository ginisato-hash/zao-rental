...
export async function startDevelopmentApp(options:{built?:boolean}={}){
 ...
 try{
  ...
  const config:DevelopmentRuntime={origin,namespace:db.identity.namespace,authSecret:randomBytes(32).toString('hex'),authDb:roles.authDb,ledgerDb:roles.ledgerDb};
