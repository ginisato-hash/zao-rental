...
export async function startDevelopmentApp(options:{built?:boolean}={}){
 ...
 try{
  ...
  const config:DevelopmentRuntime={origin,namespace:db.identity.namespace,authSecret:randomBytes(32).toString('hex'),authDb:roles.authDb,ledgerDb:roles.ledgerDb,holdDb:roles.holdDb,transferDb:roles.transferDb};
  ...
  const stop=()=>stopping??=(async()=>{try{if(!exited){web.kill('SIGTERM');const timer=setTimeout(()=>web.kill('SIGKILL'),10000);try{await exit;}finally{clearTimeout(timer);}}}finally{try{await roles!.close();}finally{await db.stop();}}})();
  return {origin,db,roles,exit,stop};
 }catch{if(roles)await roles.close();await db.stop();throw new Error('DEVELOPMENT_START_FAILED; authentication and database details withheld');}
}
