...
try {
  ...
  await check('fresh migration and concurrent replay are idempotent', async () => {
    ...
    assert.equal((await db.pool.query('SELECT count(*)::int AS n FROM foundation_migrations')).rows[0].n, 2);
  });
...
try {
  await check('failed initial migration rolls back DDL and ledger, releases lock, then recovers', async () => {
    ...
    await migrate(recovery.pool);
    assert.deepEqual((await recovery.pool.query('SELECT id FROM foundation_migrations ORDER BY id')).rows, [{ id: '0001' }, { id: '0002' }]);
