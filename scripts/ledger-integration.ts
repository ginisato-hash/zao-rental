...
try {
  const service=new LedgerService(db.pool,admin,async()=>{/* Explicit synthetic fixture boundary; normal runtime uses verifyLedgerWrite. */});const http=ledgerHandler(async()=>admin,p=>new LedgerService(db.pool,p,async()=>{/* Explicit synthetic fixture boundary; normal runtime uses verifyLedgerWrite. */}));
  ...
  await check('real-DB HTTP boundary rejects anonymous, customer, read-only staff and out-of-store access',async()=>{
    const anonymous=ledgerHandler(async()=>null,p=>new LedgerService(db.pool,p,async()=>{/* Explicit synthetic fixture boundary; normal runtime uses verifyLedgerWrite. */}));assert.equal((await anonymous(request('assets','POST',SAMPLE.assets[0].data))).status,401);
    const customer=ledgerHandler(async()=>({subject:'test-customer',role:'CUSTOMER',storeIds:['MOUNTAIN_BASE']}),p=>new LedgerService(db.pool,p,async()=>{/* Explicit synthetic fixture boundary; normal runtime uses verifyLedgerWrite. */}));assert.equal((await customer(request('assets'))).status,403);
    const staff=ledgerHandler(async()=>({subject:'test-staff',role:'STAFF',storeIds:['MOUNTAIN_BASE']}),p=>new LedgerService(db.pool,p,async()=>{/* Explicit synthetic fixture boundary; normal runtime uses verifyLedgerWrite. */}));
  ...
  await check('out-of-store update takes no Asset or pole row lock and cannot block the owning store',async()=>{
    for(const [resource,recordId,table] of [['assets',id(202),'ledger_assets'],['poles',id(302),'ledger_poles']] as const){
      ...
      const outsider=new LedgerService(controlledPool,{subject:'other-store-admin',role:'ADMIN',storeIds:['MOUNTAIN_BASE']},async()=>{/* Explicit synthetic fixture boundary; normal runtime uses verifyLedgerWrite. */});
      ...
      try {
        ...
        const owner=new LedgerService(db.pool,{subject:'owning-store-admin',role:'ADMIN',storeIds:['ONSEN_BASE']},async()=>{/* Explicit synthetic fixture boundary; normal runtime uses verifyLedgerWrite. */});
