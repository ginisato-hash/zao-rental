...
export async function provisionOperationsRole(owner:Pool,identity:{namespace:string;database:string;dbPort:number}){
 ...
 await owner.query(`GRANT EXECUTE ON FUNCTION inventory_clock(),inventory_record_replan(jsonb,jsonb),ops_assert_actor(text,text[],text),rental_apply_receipt(uuid),rental_apply_inspection(uuid),rental_complete_no_pickup(uuid),ops_checkout_amendment(uuid) TO ${user}`);
