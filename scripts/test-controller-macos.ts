...
try {
  ...
  const plan = await sandboxPlan(binding,resolve(release.directory,'worker-probe.mjs'),['boundary',resolve(release.directory,'policy.json'),state,canary,writable,String(process.pid)]);
  const result = await runBounded(release,node,plan,20_000);
  assert.equal(result.exitCode,0,result.output);
  const report = JSON.parse(result.output.trim().split('\n').at(-1)!) as Record<string,unknown>;
  for(const name of ['policyWrite','policyRename','stateWrite','outsideRead','outsideSignal','symlinkWrite']) assert.ok(['EPERM','EACCES'].includes(String(report[name])),`${name}: ${report[name]}`);
  assert.deepEqual(report.credentialEnv,[]); assert.equal(await readFile(writable,'utf8'),'allowed'); assert.equal(await readFile(state,'utf8'),'trusted');
  console.log(JSON.stringify({test:'actual installed Codex sandbox / macOS Seatbelt',codexVersion:execFileSync(codex,['--version'],{encoding:'utf8'}).trim(),runtimeHashes:plan.runtimeHashes,profileHash:plan.profileHash,sourceSha:release.manifest.sourceSha,releaseHash:release.manifestHash,denials:report,environment:'clean PATH/LANG/TMPDIR only',modelCalls:0}));
  ...
  console.log('PASS actual sandbox denies policy/state writes, rename, symlink writes, out-of-scope reads and ancestor signals; allowed workspace write succeeds.');
