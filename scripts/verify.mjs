...
const commands = [];
for (const name of ['check:reference', 'check:secrets', 'test:reference', 'lint', 'typecheck', 'test:unit', ...(process.platform === 'darwin' ? ['test:controller:macos'] : []), 'test:integration', 'test:inventory', 'test:transfer', 'build', 'test:auth', 'test:holds-ui', 'test:transfers-ui', 'test:e2e']) {
