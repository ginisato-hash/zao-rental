...
test('production ledger pages and every API operation stay denied; test app is not a production route',async({page,request})=>{
 ...
 expect((await request.get('/test-ledger')).status()).toBe(404);expect((await request.get('/tests/ui-app')).status()).toBe(404);expect((await request.get('/race')).status()).toBe(404);
});
