...
export async function startFlowApp(options:{paymentFault?:'SAVE_THEN_LOSE'}={}){
 ...
 try{
  ...
  roles=await provisionApplicationRoles(db.pool,db.identity);flow=await provisionFlowRole(db.pool,db.identity);
  ...
  const env:NodeJS.ProcessEnv={NODE_ENV:'development',PATH:process.env.PATH,NEXT_TELEMETRY_DISABLED:'1',ZAO_DEVELOPMENT_RUNTIME:JSON.stringify(config),ZAO_TEST_FLOW_RUNTIME:JSON.stringify(flow.flowDb),...(options.paymentFault?{ZAO_TEST_FLOW_FAULT:options.paymentFault}:{})};
  ...
  const stop=()=>stopping??=(async()=>{try{await current.stop();}finally{try{await flow!.close();await roles!.close();}finally{await db.stop();}}})();
  return {origin,db,roles,flow,get exit(){return current.exit;},get webPid(){return current.pid;},stop,async restartWeb(){
 ...
 }catch{if(flow)await flow.close();if(roles)await roles.close();await db.stop();throw new Error('DEVELOPMENT_START_FAILED; authentication and database details withheld');}
}
