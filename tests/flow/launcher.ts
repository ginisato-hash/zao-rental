...
export async function startFlowApp(){
 ...
 try{
  ...
  const env:NodeJS.ProcessEnv={NODE_ENV:'development',PATH:process.env.PATH,NEXT_TELEMETRY_DISABLED:'1',ZAO_DEVELOPMENT_RUNTIME:JSON.stringify(config),ZAO_TEST_FLOW_RUNTIME:JSON.stringify(flow.flowDb)};
  ...
  const stop=()=>stopping??=(async()=>{try{if(!exited){web.kill('SIGTERM');const timer=setTimeout(()=>web.kill('SIGKILL'),10000);try{await exit;}finally{clearTimeout(timer);}}}finally{try{await flow!.close();await roles!.close();}finally{await db.stop();}}})();
  return {origin,db,roles,flow,exit,stop};
