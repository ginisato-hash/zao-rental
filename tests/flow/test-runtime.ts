...
export function testFlowService(identity:FlowIdentity,guestAuth?:Pool){const {r,pool}=setup();return new BookingService(pool,guestAuth??r.authPool,identity,gateway,{merchantId:'SYNTHETIC-MERCHANT',locations:{MOUNTAIN_BASE:'SYNTHETIC-MOUNTAIN',ONSEN_BASE:'SYNTHETIC-ONSEN'},nodeEnv:'development'});}
export function testFlowOrigin(){return setup().r.config.origin;}

export function testWearService(identity:FlowIdentity){const {r,pool}=setup();if(!identity.sessionId)throw new FlowError('FORBIDDEN',403);return new WearService(pool,r.authPool,{subject:identity.subject,sessionId:identity.sessionId});}
