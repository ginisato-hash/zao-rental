...
try{
 ...
 const ledgerHttp=ledgerHandler(async()=>{const s=await resolveStaff(auth,db.pool,authHeaders);return s.status==='authorized'?ledgerPrincipal(s.principal):null;},p=>new LedgerService(db.pool,p,async()=>{/* Explicit synthetic fixture boundary; normal runtime uses verifyLedgerWrite. */}),origin);
