...
try{
 ...
 // Columns 0033/0034 add to fingerprinted tables are absent before the upgrade, so compare without them.
 const added:Record<string,string>={pricing_history:"-'reason'",rental_bookings:"-'notification_locale'"};
 ...
 console.log('PASS populated0032 to0034 preserves confirmed booking/payment/quote/HOLD/inventory/audit; replay unchanged; new permissions default deny');
 console.log(JSON.stringify({status:'PASS',oldMigrationsUnchanged:32,upgrade:'0032→0034',rollback:'transactional DDL before commit',hostedDb:0}));
