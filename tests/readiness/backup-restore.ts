...
try{
 ...
 const wrong=new Pool({host:'127.0.0.1',port:backup.identity.dbPort,...backup.auth,password:randomBytes(24).toString('hex'),max:1,connectionTimeoutMillis:2000});try{await assert.rejects(wrong.query('SELECT 1'),{code:'28P01'});}finally{await wrong.end();}
 console.log('PASS restored original credential remains authoritative; a different synthetic password is refused');
 ...
 console.log('P2 isolated backup/restore: 5 cases passed; synthetic cold same-major cluster, not production SLA or provider restore.');
