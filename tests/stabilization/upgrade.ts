...
try{
 ...
 assert.equal((await db.pool.query("SELECT kind FROM booking_actors WHERE id=$1",[id])).rows[0].kind,'STAFF');assert.equal((await db.pool.query("SELECT count(*)::int n FROM pg_constraint WHERE contype='f' AND confrelid='booking_actors'::regclass AND conrelid NOT IN ('guest_contexts'::regclass,'inventory_buffer_override_log'::regclass,'booking_cancellations'::regclass) AND convalidated")).rows[0].n,14);
