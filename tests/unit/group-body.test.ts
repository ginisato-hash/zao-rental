...
test('current schema maxima plus normal HOLD/amend/quote wrappers fit finite40KiB UTF8',()=>{
...
test('readJson counts actual split UTF8 stream, ignores declared length, cancels overflow',async()=>{
 let cancelled=false,pulls=0;const stream=new ReadableStream<Uint8Array>({pull(c){pulls++;c.enqueue(new Uint8Array(4096).fill(32));},cancel(){cancelled=true;}},{highWaterMark:0});const request=new Request('http://localhost',{method:'POST',duplex:'half',headers:{'content-type':'application/json','content-length':'1'},body:stream} as RequestInit);await assert.rejects(readJson(request,GROUP_JSON_BYTES),{status:413});assert.equal(cancelled,true);assert.equal(pulls,Math.floor(GROUP_JSON_BYTES/4096)+1);
