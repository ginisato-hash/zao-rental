...
test('additive migration plan has one ordered entry per version, no duplicates',()=>{assert.deepEqual(migrationPlan.map(m=>m.id),Array.from({length:38},(_,i)=>String(i+1).padStart(4,'0')));});
