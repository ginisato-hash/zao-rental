...
test('activation plan is deterministic, no-write and bound to the canonical migration set and role plans',async()=>{
 ...
 assert.equal(a.credentialActivation.version,'production-credential-activation/2');assert.equal(a.credentialActivation.passwordAuthority,'NEON_ROLE_RESET_PASSWORD_API');assert.equal(a.credentialActivation.roleAttributeAuthority,'neondb_role_admin');assert.equal(a.credentialActivation.passwordSqlTransport,'NONE');assert.equal(a.credentialActivation.passwordReset.idempotent,false);assert.equal(a.nextWriteStep.gate,'PRODUCTION_CREDENTIAL_CANARY');assert.doesNotMatch(JSON.stringify(a),/SCRAM|scram_iterations/);assert.equal(a.credentialActivation.managerRole,'neondb_role_admin');assert.equal(a.credentialActivation.canary,'content_read');assert.equal(a.credentialActivation.services.length,10);assert.equal(a.credentialActivation.branchProtection,'REQUIRED_BEFORE_FIRST_CREDENTIAL');assert.match(a.credentialActivation.planSha256,/^[a-f0-9]{64}$/);
