...
/** R10 controlled operator guard, deliberately independent of the old P4 journal.
...
export async function dispatchS3Once(path,identity,send){
 if(!identity||!Object.values(identity).every(v=>typeof v==='string'&&v.length>0)||typeof send!=='function')throw new Error('S3_GUARD_INVALID');
